  ...
  output$overlap <- renderPlot({
      if (grepl('PSS', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
        ...
        ggplot(data, aes(x=Moment, y=PSS, fill=PMS))+
          ...
          # 
          # {if(noPMS1_noPMS2()!='')geom_segment(aes(x =0.9, y = 3, xend = 1.9, yend = 3), size= 1, colour= "#2166AC")}+
          ...
          # {if(PMS1_PMS2()!='')geom_segment(aes(x =1, y = 3.3, xend = 2, yend = 3.3), size= 1, colour= "#49A4B9")}+
          ...
          # {if(PMDD1_PMDD2()!='')geom_segment(aes(x =1.1, y = 3.6, xend = 2.1, yend = 3.6), size= 1, colour= "#92C5DE")}+
          ...
          geom_point(data = dmean, aes(x = as.numeric(Moment), y = PSS, group = PMS, fill = PMS), position= position_dodge(0.3), shape = 21, size=10)+
...
#     )
        ...
        # "red1", "#2C5985","#49A4B9"
          ...
          ggplot(data, aes(x = PMS, y = PSS)) +
            ...
            # geom_segment(aes(x = 1, y=3, xend= 2, yend=3), size= 1.7)+
            # geom_segment(aes(x = 2, y=2.5, xend= 3, yend=2.5), size= 1.7)+
            # geom_segment(aes(x = 1, y=4, xend= 3, yend=4), size= 1.7)+
            # # annotate('text', x=1.5, y=3.4, label="**", size=10)+
            # annotate('text', x=1.5, y=3.4, label=sprintf("\n%s\n", noPMS_PMS()), size=10)+
            # annotate('text', x=2.5, y=2.9, label=sprintf("\n%s\n", PMS_PMDD()), size=10)+
            # annotate('text', x=2, y=4.4, label=sprintf("\n%s\n", noPMS_PMDD()), size=10)+
      ...
      geom_segment(aes(x = 1.955, y = (max_y+(max_y/10)-(max_y/80)), xend = 2.055, yend = (max_y+(max_y/10)-(max_y/80))), size= 2)+
        ...
        scale_fill_manual(values = c("red1", "#49A4B9", "#2C5985"))+
          ...
          ggplot(data, aes(x = PMS, y = BSRI)) +
            ...
            geom_point(data = dmean, aes(x = as.numeric(PMS), y = BSRI, group=PMS, fill=PMS), shape = 21, size=6)+
    ...
    output$overlap <- renderPlot({
      if ((grepl("Stress", input$formula_choice))){
          ...
          ggplot(data, aes(x = PMS, y = DASS_Stress)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = 0), adjust = 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25, alpha= .5)+
            geom_boxplot(aes(x = PMS, y = DASS_Stress, fill = PMS),outlier.shape= NA, width = .1, colour = "black", alpha= .5)+
          ...
          scale_fill_manual(values = c("red1", "#49A4B9", "#2C5985"))+
            scale_colour_manual(values = c("red1", "#49A4B9", "#2C5985"))+
            theme_bw()
          #   geom_segment(aes(x = 1, y=max_y+(max_y/10)+(max_y/130), xend = 1.98, yend=max_y+(max_y/10)+(max_y/150)), size= 1.7)+
          # annotate('text', x=1.6, y=max_y+(max_y/10)+(max_y/80), label="***", size=6)+
          # geom_segment(aes(x = 2.02, y=max_y+(max_y/10)+(max_y/130), xend = 3, yend=max_y+(max_y/10)+(max_y/150)), size= 1.7)+
          # annotate('text', x=2.6, y=max_y+(max_y/10)+(max_y/80), label="***", size=6)
...
server <- function (input, output){ #we have to add this to the server
  ...
  output$overlap <-renderPlot({
          ...
          ggplot(data, aes(x = PMS, y = allRRS)) +
            ...
            # geom_segment(aes(x = 1, y=max_y+(max_y/10)+(max_y/200), xend = 2, yend=max_y+(max_y/10)+(max_y/200)), size= 1.7)+
          # annotate('text', x=1.6, y=max_y+(max_y/10)+(max_y/100), label=sprintf("\n%s\n", p_pms()), size=10)+
            # annotate('text', x=1.6, y=max_y+(max_y/10)+(max_y/100), label="***", size=10)+
            scale_x_discrete(labels=c('1'='follicular', '2'='premenstrual'))+
            ggtitle('allRRS~PMS')+
          scale_fill_manual(values = c("red1", "#49A4B9", "#2C5985"))+
            scale_colour_manual(values = c("red1", "#49A4B9", "#2C5985"))+
...
-standardize the variables to get standardized coefficients with scale ()
https://stackoverflow.com/questions/24305271/extracting-standardized-coefficients-from-lm-in-r

doe het met if standardized = 1 then standardize continue variabelen else doe niet 

-number of datapoints toevoegen valence/arousal plots



bekijk associatie ruminatie en stress? correlations?
hoe relateerd BSRI aan de stress? wss veel, wss zelfde participanten

 

symptomen plotten over tijd en controleren traits

corona measures: even extraheren en visualiseren: zit er iets interessant? 

tijdchart en eroverheen lockdown lockdown (per NL en BE)
- land/nationality
-PTQ

ook samen met PMS noPMS en testmoment 1 en 2. wordt zowiezo genormaliseerd over de tijd. 
wss mensen met PMS meer last v corona en meer tijdens moment 2. 
mss moet tijd/date vervangen door 'lockdown'. oudste datum 1 nieuwste datum 1000 (voor elke dag 1) en dan plotten over tijd. 
zou ook lockdown 1/0 kunnen zijn. 
mss negatief effect tijd: na tijdje minder effect v corona op piekergedachten. steeds minder prominente plek in in mensen hun brein. 
mr als definieren als wel lockdown geen lockdown: als blocks? welke variabelen daar afkaderen? 
=> doe maar hypothesegedreven. welke factoren logisch om invloed te hebben? 
bv aantal kinderen? kijk nr variabelen om iets interessant te vinden. maanden post-partum? meer piekeren?? 


PTQ ~ DATE
is goed eerst begin 

- meer interactieve code maken 
- meer commenten 

...
server <- function (input, output){ #we have to add this to the server
  ...
  output$table <- renderTable ({
    ...
    tabel <- cbind(tabel,chosenModel)
    #tabel <- matrix(cbind(tabel,chosenModel), ncol=10,byrow=FALSE)
    
    
    colnames(tabel)= c('d0.1','d0.2','d0.3','d0.4','d0.5','d0.6','d0.7','d0.8','d0.9','chosenModel')
    tabel

    #rownames(tabel)<-c('d0.1','d0.2','d0.3','d0.4','d0.5','d0.6','d0.7','d0.8','d0.9')
    #colnames = c(d0.1,d0.2,d0.3,d0.4,d0.5,d0.6,d0.7,d0.8,d0.9)
...
TO DO:
-vraag Mitchel doorlopen variabelen
-Tilia Rstudio
-Dass subschalen: in beeld, statistiek etc
-op apart tabblad
-stimuli plotten, outliers eruit 

PMS no PMS: leeftijd, duur cyclus
ondvr 1: stress hoger luteale fase dan follicululaire fase? 
ondvr 2/ rumination higher in luteale phase than folliculair with PMS vs no PMS? 

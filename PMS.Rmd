...
A lot of women feel unwell during the premenstrual phase of their cyclus. 
...
![](figures/BSRI_norms.png){width=80%}
...
!["Image Source: https://www.psytoolkit.org/survey-library/depression-anxiety-stress-dass.html"](figures/MC.png){width=80%}
    ...
    if (grepl('Arousal', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
      dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('PMSScoreNew', 'Moment'))
        p3 <- ggplot(dataBig2, aes(x=Moment, y=Arousal, fill=PMSScoreNew))+
          ...
          scale_x_discrete(labels=c('1'='luteal', '2'='follicular'))+
          annotate('text', x=.73, y=60, label=sprintf("* {", size=5), colour='red')+
          annotate('text', x=1.73, y=60, label=sprintf("* {", size=5), colour='red')+
          annotate('text', x=1.5, y=85, label=sprintf("*", size=5), colour='red')+
          geom_segment(aes(x = 1.025, y = 80, xend = 1.98, yend = 80), colour='red')+
      ...
      dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('PMSScoreNew'))
          ...
          annotate('text', x=1.73, y=80, label=sprintf("\n%s\n", p_pms()), size=5, colour='red')+
    ...
    if (grepl('Arousal', input$formula_choice)){
      dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('PMSScoreNew', 'Moment'))
        p<- ggplot(dataBig2, aes(x=Moment, y=Arousal, fill=PMSScoreNew))+
          ...
          geom_boxplot(aes(x = Moment, y = Arousal, fill = PMSScoreNew),outlier.shape= NA, alpha = .5, width = .1, colour = "black")+
          geom_point(data = dmean, aes(x = as.numeric(Moment), y = Arousal, group = PMSScoreNew, fill = PMSScoreNew), shape = 21, size=4)
        p
      ...
      dmean<-summarySE(dataBig2, measurevar="Valence", groupvars=c('PMSScoreNew', 'Moment'))
        p<- ggplot(dataBig2, aes(x=Moment, y=Valence, fill=PMSScoreNew))+
            geom_split_violin(alpha=.5)+
          geom_boxplot(aes(x = Moment, y = Valence, fill = PMSScoreNew),outlier.shape= NA, alpha = .5, width = .1, colour = "black")+
          geom_point(data = dmean, aes(x = as.numeric(Moment), y = Valence, group = PMSScoreNew, fill = PMSScoreNew), shape = 21, size=4)
        p
    }
  })
...
- no sign diff for Valence in PMS vs no PMS

- no sign diff Valence/Arousal PMS no PMS in interaction TestMoment

- for Arousal: noPMS1-PMS1, PMS1-noPMS2 and noPMS2-PMS2 sign contrasts
...
A_B <- length(data$Order=='A-B')
B_A <- length(data$Order=='B-A')
...
the **A-B** order contains `r A_B` trials and **B-A** `r B_A` trials: they are counterbalanced
...
bij STIMULI
-significantie: van boven, met rechte haakjes, dikker en zwart. 
-mr niet meer voor de kriskross: deze weglaten
-ipv testmoment1/2: control (1) en premenstrual
-dan: toon deze aan Mitchel

Rest:
-pas de violin plots toe
-check de raincloud plots: er staat foutief PMS vs noPMS terwijl er testmoment moet staan!
-check of alle plots nog werken
-shiny: dubbelklik zodat fullscreen/hele shiny app groter: zichtbaarheid!


STIMULI CORR:
-zet alles op dezelfde schaal: autoscale uit, altijd 0-10
-bij SD: ook /10 gedaan? staan norm PMS op dezelfde schaal??
-voeg een 'perfecte' correlatie lijn toe op de plot
-geef de stimuli kleurtjes: rood voor alle stimuli die A zijn. om te zien: ga nr data, hier kan je zien als A en moment=1

bij PSS:
-check of de vragen PSS zelfde zijn bij de norms voor corona zelfde vragen hebben gebruikt: zelfde soort PSS gebruikt? 
NORMERINGEN
-voeg tabellen/afbn toe met de normeringen zodat plots meer betekenis

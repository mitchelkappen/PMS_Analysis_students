...
#install packages
list.of.packages <- c("lme4",'lmerTest','effects','dplyr', 'car', 'emmeans','fitdistrplus','ggplot2', 'ggstatsplot','ggsignif', 'ggformula','ggdist','ggpubr','ggeffects', 'gridExtra','tidyverse','pander','stringr','cowplot','lavaan','readr','jpeg','shiny','reshape2', 'yarrr','knitr' )
...
these already have the rt data filtered under 12 939 ms (outliers removed)
...
dataBig2 <- subset(dataBig, rt < 12939)
...
# some code to find the subject ID's of the outliers
#doesn't work yet!!

# length(unique(dataBig$newid)) #how many participants
# 
# stim=c(4658)# here you fill in the number of the stimulus
# d2<-data.frame(valence=c(dataBig2$Valence[dataBig2$Stimulus==stim]),arousal=c(dataBig2$Arousal[dataBig2$Stimulus==stim]), ID=c(dataBig2$newid[dataBig2$Stimulus==stim])) #just make this d in code


# #min
# min<-d2$ID[which(d2$valence==min(d2$valence))]
# #max
# max<-d2$ID[which(d2$valence==max(d2$valence))]
# 
# if (length(min)<length(max)){
#   print(min)
# } else {
...
dataBig_out <- subset(dataBig_out, rt < 25000)
...
dataBig_out <- subset(dataBig_out, rt < 15000)
d<-melt(data.frame(rt=c(dataBig_out$rt)))
...
#pay attention! this is what we now make dataBig, and this will ALSO affect the Stimulus plotting
dataBig2 <- subset(dataBig_out, rt < 12939)
d<-melt(data.frame(rt=c(dataBig2$rt)))
...
dataBig_out <- dataBig_out%>%slice(out_ind)
d<-melt(data.frame(rt=c(dataBig_out$rt)))
p5 <- ggplot(d, aes(x = variable, y = value, fill=variable))+
  geom_flat_violin(position = position_nudge(x = .2, y = 0),adjust =2)+
...
-look at the weird answers in the valence: same participants? note their IDs
-variabelen steekproef in tabel (puur omdat  interessant: 
PMS vs no PMS: 
-leeftijd 
-duur cyclus (gemm))

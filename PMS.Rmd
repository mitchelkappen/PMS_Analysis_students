...
lower_bound <- quantile (dataBig_out$rt, 0.025)
print('lower bound')
lower_bound
upper_bound <- quantile (dataBig_out$rt, 0.975)
print('upper bound')
upper_bound

defaultW<-getOption("warn")
options(warn=-1)# this disables warning messages
dataBig_out <- subset(dataBig_out, rt < 16)
...
server <- function (input, output){ 
  ...
  output$plotmeans <-renderPlot({
      dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('PMSScoreNew', 'TestMoment'))
        p1 <- ggplot(dataBig2, aes(x = PMSScoreNew, y = Arousal)) +
          geom_flat_violin(aes(fill=PMSScoreNew),position = position_nudge(x =.2, y = -1), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA)+
          geom_point(aes(colour='red'),position=position_jitter(width=.15), size=.25)+
          geom_boxplot(aes(x = PMSScoreNew, y = Arousal, fill = PMSScoreNew),outlier.shape= NA, alpha = .5, width = .1, colour = "black")+
          geom_point(data = dmean, aes(x = as.numeric(PMSScoreNew)+.2, y = Arousal, group = PMSScoreNew, colour = PMSScoreNew), shape = 18, size=4)+
          geom_line(data=dmean, aes(x=as.numeric(PMSScoreNew)+.2, y=Arousal), size=1)+
          geom_errorbar(data = dmean, aes(x = as.numeric(PMSScoreNew)+.2, y = Arousal, group = PMSScoreNew, colour = PMSScoreNew, ymin = Arousal-se, ymax= Arousal+se), width = .05) +
  ...
  output$table <- renderTable ({
    modelNames <- c()
    models <- c()
    # Formula <- 'Valence ~ PMSScoreNew + (1|Subject)'
    Formula <- paste0(input$formula_choice, '+ (1|Subject)')
    tryCatch(
  ...
  output$Anova <- renderPrint({
    models <-c()
        Formula <- paste0(input$formula_choice, ' + (1|Subject)')
         tryCatch(
      { d0.1 <- lmer(Formula,data=dataBig2);
      models <- c(models, d0.1)}, 
      error=function(e){})
    tryCatch(
      { d0.2 <- glmer(Formula,data=dataBig2, family = gaussian(link = "inverse"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); 
...
```
#### Conclusions
-no sign diff in Valence PMS vs no PMS
-sign diff in Arousal PMS vs no PMS
-no sign diff Valence PMS no PMS in interaction TestMoment
-no sign Arousal PMS x TestMoment


### further data processing

- divide stimuli groups on valence (read about IAP images)

- compare to norm groups for IAP images, compare if sign differences
...
-connected raincloudplot maken
-split violin plots voor distributie
-connected plots repeated measures-factorial (eextended) pg 17/48²

-probeer uit in kleiner docu: output variable maken binnen de shiny app. 
ook nr buiten shiny app? hoe werkt dat? => code opkuisen
maak functies met return voor herhaalde code 
global variables, output$, return functies


bestand participants excel, in PMS 24/8/2021, op de share, date directories. daar deze datum participant excel. daarin participanten, sommige regels rood, deze exclusie. kolom AG staat vaak reden gemist. soms dr luteaal gemist, vaak meeste geval, of niet meer deelnemen. of foutje onze kant. 
met inlezen file csv obv datum 

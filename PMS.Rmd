...
data$PSS<-(data$PSS- min(data$PSS, na.rm=TRUE))/(max(data$PSS, na.rm=TRUE)-min(data$PSS, na.rm=TRUE))
data$BSRI<-(data$BSRI- min(data$BSRI, na.rm=TRUE))/(max(data$BSRI, na.rm=TRUE)-min(data$BSRI, na.rm=TRUE))
data$allRRS<-(data$allRRS- min(data$allRRS, na.rm=TRUE))/(max(data$allRRS, na.rm=TRUE)-min(data$allRRS, na.rm=TRUE))
data$DASS_Depression<-(data$DASS_Depression- min(data$DASS_Depression, na.rm=TRUE))/(max(data$DASS_Depression, na.rm=TRUE)-min(data$DASS_Depression, na.rm=TRUE))
data$DASS_Anxiety<-(data$DASS_Anxiety- min(data$DASS_Anxiety, na.rm=TRUE))/(max(data$DASS_Anxiety, na.rm=TRUE)-min(data$DASS_Anxiety, na.rm=TRUE))
data$DASS_Stress<-(data$DASS_Stress- min(data$DASS_Stress, na.rm=TRUE))/(max(data$DASS_Stress, na.rm=TRUE)-min(data$DASS_Stress, na.rm=TRUE))
...
data$PSS1 <- (lapply(data$PSS, min_max_norm))
  ...
  output$overlap <-renderPlot({
    if (grepl('arousal', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
     ...
     models <- reactive({
        ...
        tryCatch({ d0.2 <- glmer(Formula,data=datafaces, family = gaussian(link = "inverse"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d1=d0.2)}, error=function(e){})

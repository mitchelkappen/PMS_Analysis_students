...
server <- function (input, output){ #we have to add this to the server
  ...
  # https://www.rpubs.com/jenrichmond/W6LL  here some info on the following plot
    output$raincloud2 <- renderPlot({
      if ((grepl("PSS ~ PMSScoreNew*TestMoment", input$formula_choice))){

        dataf<- data
        # we create a dataframe with only PMSScoreNew, TestMoment and BSRI
        require(reshape2)
        dataf$id <- rownames(data)
        dataf<- melt(dataf, id=c("TestMoment", "PSS", "PMSScoreNew"))
        head(dataf)

        # now we need to make it so that array1= Testmoment 1, no PMS
        #array2= Testmoment1, PMS,
        #array3= Testmoment2, no PMS
        #array4= Testmometn2, PMS

        Mom1_noPMS<-subset(dataf, PMSScoreNew!="PMS"& TestMoment!='2')
        Mom1_PMS <- subset(dataf, PMSScoreNew!="noPMS"& TestMoment!='2')
        Mom2_noPMS <-subset(dataf, PMSScoreNew!="PMS"& TestMoment!='1')
        Mom2_PMS <- subset(dataf, PMSScoreNew!="noPMS"& TestMoment!='1')
      ...
      if ((grepl("BSRI ~ PMSScoreNew*TestMoment", input$formula_choice))){

        dataf<- data
        # we create a dataframe with only PMSScoreNew, TestMoment and BSRI
        require(reshape2)
        dataf$id <- rownames(data)
        dataf<- melt(dataf, id=c("TestMoment", "BSRI", "PMSScoreNew"))
        head(dataf)
      ...
      else if ((grepl("PSS", input$formula_choice))){
        ...
        require(reshape2)
        dataf$id <- rownames(data)
        dataf<- melt(dataf, id=c("TestMoment", "PSS"))
        head(dataf)
        
        df <-data_1x1(
          array_1= dataf$PSS[1:2135],
          array_2=dataf$PSS[2135:4270],
          jit_distance=0.09,
          jit_seed=321)
        ...
        raincloud_2
                ...
                #dataframe for connected rainclouds
        dataf<- data
        # we create a dataframe with only PMSScoreNew and BSRI
        require(reshape2)
        dataf$id <- rownames(data)
        ...
        df <-data_1x1(
          array_1= dataf$BSRI[1:2135],
          array_2=dataf$BSRI[2135:4270],
          jit_distance=0.09,
          jit_seed=321)

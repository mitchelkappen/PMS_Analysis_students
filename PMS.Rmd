...
} else {Dir="C:\\Users\\ASUSTeK\\OneDrive\\Documenten\\GitHub" #data from github dir
}
...
# datafaces<-subset(data, confidence >.95)#only take data where confidence is bigger than 95

#remove datafaces with NA varaibles
# datafaces<-na.omit(datafaces)

datafaces$VideoValence <-data$happy
...
- zet tilia liese marie-anne en steven weyers op de pp! 

- 3 testmomenten: intake/screening, tesmoment 1 en 2 (herlees Tilias masterproef: methode en stappen). tijdens screening sociaal demografische gegevens+ trait questionnairs DASS 

- tussenhoofdstukje: methode. meer ingaan op bv ruminatie. overzichtje van alle vragenlijsten en mss voorbeeld v hoe trial eruit ziet: afbeelding, reactie. technische info: hoeveel seconden, welke dingen allemaal beoordelen, (valence arousal)

-EINDIG met DASS: trait stress anxiety depression is higher: universal PMS vs noPMS
-plots op slides over heen typen titels moment etc

-maak aparte slides van de correlaties want nu te klein!

- zet statistieken erbij: grootte p (zoek op laatste publicatie Mitchel/jens wat gerapporteerd en uitlezen uit emmeans/anovatabellen)

-opslaan Rmd files png automatisch! 

hoofdstukjes: stress, vragenlijsten voor stress/ resultaten stress

slide voor de plots met descriptives: hoeveel data , n participanten, alpha
bij gezichtenn: meld dat minder data! 
...
- deze tool is gematched met diagnose die doktoren wel kunen geven en prevalenties in die groepen kwamen overeen: meest betrouwb screeningstool die er op deze moment is. 

- ondersch: geen tot weinig, mild tot sterk en ernstige klachten. laatste twee samen. PMDD kan enkel psychiater vaststellen. 

-bij testmoments: zet afb nog eens bij 
-pp emotiµes: im^plicit= sliders explicit= emotional expressions

- maak PMDD minder centraal!! das psychiatrische aandoening, niet noemen. enkel noemen ifv dat het meer aandacht krijgt nu sinds opgenomen in DSM: meer erkenning voor PMS klachten ; zoals bij veel dingen DSM versie prevalentie vrij laag  maar veel vrouwen hebben evengoed symptomen, 20-30 sprake van PMS (geen officiele diagnose moeilijk af te kaderen)

- gebruik andere afbeelding v helloclue

- zet N op de corr

- doe valence en arousal!  plot? per groep? 


- er komt een kolom 'confidence' en bv only use facial data when >.95

- change videovalence to happy

- emoties 2x2 plotten: emoties op time *group

- ArousalVideo plotten en correlatie met Arousal
...
server <- function (input, output){ 
  #we make a density plot of valence/arousal per stimulus.
    ...
    output$ValenceCorrelation<- renderPlot({
      ...
      ggscatter(datafaces, x = "Valence", y = "happy", 
      add = "reg.line", conf.int = TRUE, 
      xlab='Valence', ylab='VideoValence',
      cor.coef = TRUE, cor.method = "pearson",
      title=sprintf("\ndatapoints: %s, n: %s\n", datapoints, n))# NAs in AU12 & AU15
      # geom_segment(aes(x = 0, y = 0, xend = 100, yend = 2), size= 1, colour='red')
      # library('ggpubr')
      # ggqqplot(datafaces$AU12)
    }, height=1000, width=1500)
  ...
  output$ArousalCorrelation<- renderPlot({
      ...
      datapoints<-length(datafaces$AU12-length(NAs))
      n<- length(unique(datafaces$ID))
      ggscatter(datafaces, x = "Arousal", y = "arousal", 
      add = "reg.line", conf.int = TRUE, 
      xlab='Arousal', ylab='VideoArousal',
      cor.coef = TRUE, cor.method = "pearson",
      title=sprintf("\ndatapoints: %s, n: %s\n", datapoints, n))# NAs in AU12 & AU15
      # geom_segment(aes(x = 0, y = 0, xend = 100, yend = 2), size= 1, colour='red')
      # library('ggpubr')
...
if ( "scared" %in% input$Emotion) return(datafaces$scared)
  ...
  output$plot <-renderPlot({
    ...
    # dmean<-summarySE(datafaces, measurevar='sad', groupvars=c('Moment', 'PMS'), na.rm=TRUE)
    # ggplot(datafaces, aes(x=Moment, y=sad, fill=PMS))+
    #       geom_flat_violin (aes(fill=PMS), position= position_nudge(x=.1, y=0), adjust=1.5, alpha= .5, colour=NA)+
    #       geom_boxplot(aes(x=Moment, y=sad, fill=PMS), outlier.shape=NA, alpha=.5, width=.1, colour='black')+
    #       geom_point(data = dmean, aes(x = as.numeric(Moment)+.1, y = sad, group = PMS, fill= PMS), shape = 21, size=6)+
    #       scale_x_discrete(labels=c('1'='follicular', '2'='premenstrual'))

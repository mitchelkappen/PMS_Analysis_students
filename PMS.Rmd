...
# doen zodat  kiezen tussen lijst van stimuli

ui<- fixedPage( 
  title= "plotting Stimuli",
  fluidRow(
    radioButtons("Stimulus", label="Choose a Stimulus:",
            choices= c('1440', '1463', '1610', '1710', '1722', '1750', '2057', '2070', '2160', '2165', '2340', '2360', '2800', '3261', '4180', '4290', '4490', '4531',  '4550', '4561', '4658', '5030', '5600', '5800', '5982', '6312', '6313', '7182', '7211', '7224', '7238', '7380', '7480', '7481', '9220', '9300', '4550', '4561', '4658', '5030', '5600', '5800', '5982', '6312', '6313', '7182', '7211', '7224', '7238', '7380', '7480', '7481', '9220', '9300'), inline=TRUE,
            selected='1440'), ),
  #we need to make this 2 plots under each other, for later
...
server <- function (input, output){ 
  ...
  output$densityplot<- renderPlot({
    ...
    #we maken density plot van valence/arousal per stimulus. 
    #for i in unique(kolom)
    
    d <- c()
    
    for (i in 1:length(dataBig$newid)){
      if (dataBig$Stimulus[i]==input$Stimulus){
        d<-c(d, dataBig$Valence[i])
      }
    }
    # for (i in 1:15796){if (dataBig$Stimulus[i]==1710){d<-c(d, dataBig$Valence[i])}}
    d<-data.frame(valence=c(d))
    
    p1 <- ggplot(d, aes(x=valence ))+
      geom_density()
    #also add description valence/arousal
    p1
...
# for (i in 1:length(dataBig$newid)){
#   dataBig$Valence[databig$newid==i & dataBig$Stimulus==input$stimulus]
#  #mean of valence for testMoment 2
#   dataBig$val_2[i]<-mean(dataBig$Valence[dataBig$newid==i & dataBig$TestMoment==2])
# }
# #plot 
# 
# 
# 
# #voor analyse later: per participant gemm voor alle stimuli voor arousal en zelfde voor valence
# 
# # we index over the dataset to get the mean valence and arousal per participant per testmoment
# 
# for (i in 1:length(dataBig$newid)){
#   id_nr=i
#   for (j in 1:2) {
#     mean(dataBig$Valence[dataBig$newid==j &dataBig$TestMoment==j])}
# }
# 
# for (i in 1:length(dataBig$newid)){
#  #mean of valence for testMoment 2
...
#plot with testmoment 1, testmoment 2: valence 
Formula <- Valence ~ TestMoment
   dpi=600    #pixels per square inch
    # jpeg(paste0(plotPrefix, "Figure", "_", plotTitles[i], ".jpeg"), width=8*dpi, height=4*dpi, res=dpi)
    par(mfcol = c(1, 1))
      pirateplot(

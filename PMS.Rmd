    ...
    output$overlap <- renderPlot({
      if ((grepl("Depression", input$formula_choice))){
        ...
        ggplot(data, aes(x=PMS, y=DASS_Depression))+
              ...
              geom_point(data=frame, aes(colour=PMS),position=position_jitter(width=.15), alpha=.5, size=1)+
...
server <- function (input, output, session){ 
 ...
 output$table <-renderTable({
  ...
  noPMSframe <- data.frame (noPMS_PSS, noPMS_BSRI, noPMS_1_PSS, noPMS_1_BSRI, noPMS_2_PSS, noPMS_2_BSRI, noPMS_delta_PSS, noPMS_delta_BSRI)

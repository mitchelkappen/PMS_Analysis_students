...
as.data.frame(norms) # make a dataframe
...
ui<- fluidPage( 
  sidebarPanel(
    radioButtons("choice", label="data from normgroups or from this study?",
            choices= c('norms', 'PMS'), inline=TRUE,
            selected='norms'), ),
  mainPanel(
    tabsetPanel(type = "tab",        
...
server <- function (input, output){ 
  plot <-renderPlot({
    if ((grepl("norms", input$choice))){
        p1 <- ggplot(norms, aes(x = valmn, y = aromn))+
          geom_point()+
...
# calculate correlations
    cor(norms$valmn, norms$valmn_pms, method=c('pearson'))
    ...
    ggscatter(norms, x='valmn', y='valmn_pms', add='reg.line', conf.int=TRUE,
        ...
        ggscatter(norms, x='arosd', y='arosd_pms', add='reg.line', conf.int=TRUE,
              cor.coef=TRUE, cor.method='pearson', 
              xlab='norm SD Arousal', ylab='PMS SD Arousal')
        
        #perhaps: pirateplot van de Arousal norms en Arousal pms
...
-conclusions opschrijven

-devide based on valence, arousal
-zelfde analyses

-normgroepen IAPs en normgroepen PSS

...
datafaces$VideoValence= datafaces$AU12 - datafaces$AU15
...
server <- function (input, output){ 
  #we make a density plot of valence/arousal per stimulus.
    ...
    output$ValenceCorrelation<- renderPlot({
      ...
      n<-length(datafaces$AU12-length(NAs))
      ggscatter(datafaces, x = "Valence", y = "VideoValence", 
...
if ( "scared" %in% input$Emotion) return(datafaces$scared)
  ...
  dmean<-reactive({
    summarySE(datafaces, measurevar=input$Emotion, groupvars=c('PMS', 'Moment'))
  })
  ...
  output$plot <-renderPlot({
    # dmean<-summarySE(datafaces, measurevar='happy', groupvars=c('PMS', 'Moment'))
    ...
    dmean<-summarySE(datafaces, measurevar='sad', groupvars=c('PMS', 'Moment'))

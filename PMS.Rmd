...
server <- function (input, output){ #we have to add this to the server
  ...
  output$table <- renderTable ({
    ...
    tryCatch(
      { d0.1 <- lmer(Formula,data=data); models <-c(models,d0.1) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-1]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.2 <- lmer(Formula,data=data); models <-c(models,d0.2) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-2]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.3 <- lmer(Formula,data=data); models <-c(models,d0.3) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-3]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.4 <- lmer(Formula,data=data); models <-c(models,d0.4) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-4]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.5 <- lmer(Formula,data=data); models <-c(models,d0.5) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-5]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.6 <- lmer(Formula,data=data); models <-c(models,d0.6) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-6]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.7 <- lmer(Formula,data=data); models <-c(models,d0.7) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-7]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.8 <- lmer(Formula,data=data); models <-c(models,d0.8) }, # if this formula works, we print it
      error=function(e){
        modelNames <-modelNames[-8]
        })# if it doesn't, then we set this to true
        tryCatch(
      { d0.9 <- lmer(Formula,data=data); models <-c(models,d0.9)
        modelNames <-modelNames[-9]
        })# if it doesn't, then we set this to true

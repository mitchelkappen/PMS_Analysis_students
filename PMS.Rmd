...
if (vpn == 1){Dir = "Z:\\shares\\ghepmk_data\\2020_Kappen_PMS\\RMarkdown\\" #data from VPN folder
} else {Dir = "C:\\Users\\ASUSTeK\\OneDrive\\DocumentenGitHub\\internship_sofie\\" #data from github dir
}
setwd(Dir)
data <- read.csv(paste0(Dir,"Data\\allPMSData.csv"), header=TRUE, sep=) #upload data
...
!["Image Source: https://helloclue.com/articles/cycle-a-z/the-menstrual-cycle-more-than-just-the-period"](figures/MC.png){width=60%}
...
![](figures/screening.png){width=50%}
...
#### follicular vs premenstrual phase:
...
ui<- fluidPage( # makes the User Interface
    selectInput("formula_choice", label="Choose a Formula:",
     ...
     tabsetPanel(type = "tab",
                 ...
                 tabPanel("TestMoment_distribution", plotOutput("raincloud2")),
    ...
    output$overlap <- renderPlot({
      ...
      max_y<-max(as.integer(data$PSS))
          ...
          ggplot(dataBig2, aes(x = PMS, y = PSS)) +
            ...
            geom_segment(aes(x = 1.2, y=max_y+(max_y/10)+(max_y/130), xend = 2.2, yend=max_y+(max_y/10)+(max_y/130)), size= 1.7)+
...
ui<- fluidPage( # makes the User Interface
    selectInput("formula_choice", label="Choose a Formula:",
     ...
     tabsetPanel(type = "tab",
                  tabPanel("pirateplot", plotOutput("pirateplot")),
                  tabPanel("overlap", plotOutput("raincloud")),
                 tabPanel("splitviolin", plotOutput("splitviolin")),
                 tabPanel("AIC table", tableOutput("table")),
                 tabPanel("Anova", verbatimTextOutput("Anova")),
                 tabPanel("emmeans",verbatimTextOutput("emmeans"))
  ...
  output$pirateplot<- renderPlot({
    ...
    output$raincloud <- renderPlot({
      if ((grepl("Stress", input$formula_choice))){
          ...
          ggplot(dataBig2, aes(x = PMS, y = DASS_Stress)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = -1), adjust = 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
          ...
          ggplot(dataBig2, aes(x = PMS, y = DASS_Anxiety)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = -1), adjust = 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
          ...
          ggplot(dataBig2, aes(x = PMS, y = DASS_Depression)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = -1), adjust = 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
...
server <- function (input, output){ 
  ...
  output$densityplot<- renderPlot({
    d<-melt(data.frame(valence=c(dataBig2$Valence[dataBig2$Stimulus==input$stimulus]),arousal=c(dataBig2$Arousal[dataBig2$Stimulus==input$stimulus]), order=c(dataBig2$Order[dataBig2$Stimulus==input$stimulus])))
...
pirateplot(
  formula=Arousal ~ Order,
  data=dataBig2,
  pal='xmen')

pirateplot(
  formula=Valence ~ Order,
  data=dataBig2,
  pal='xmen')
```
...
server <- function (input, output){ 
  ...
  output$plotmeans <-renderPlot({
    if (grepl('Arousal', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
      dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('PMS', 'Moment'))
        ggplot(dataBig2, aes(x=Moment, y=Arousal, fill=PMS))+
          geom_flat_violin (aes(fill=PMS), position= position_nudge(x=.1, y=0), adjust=1.5, alpha= .5, colour=NA)+
      ...
      dmean<-summarySE(dataBig2, measurevar="Valence", groupvars=c('PMS', 'Moment'))
        ggplot(dataBig2, aes(x=Moment, y=Valence, fill=PMS))+
          geom_flat_violin (aes(fill=PMS), position= position_nudge(x=.1, y=0), adjust=1.5, alpha= .5, colour=NA)+
      ...
      dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('PMS'))
          ggplot(dataBig2, aes(x = PMS, y = Arousal)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = -1), adjust = 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
      ...
      dmean<-summarySE(dataBig2, measurevar="Valence", groupvars=c('PMS'))
          ggplot(dataBig2, aes(x = PMS, y = Valence)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = -1), adjust= 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
  ...
  output$splitviolin <-renderPlot({
      if ((grepl("Arousal", input$formula_choice)==TRUE)&!(grepl("TestMoment", input$formula_choice))){
        max_y<-max(as.integer(dataBig2$Arousal))
        dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('PMS'))
        ggplot(dataBig2, aes(x=1, y=Arousal, fill=PMS))+
            geom_split_violin(adjust =1.5, alpha=.5)+
      ...
      dmean<-summarySE(dataBig2, measurevar="Valence", groupvars=c('PMS', 'Moment'))
        ...
        max_y<-max(as.integer(dataBig2$Valence))
        dmean<-summarySE(dataBig2, measurevar="Valence", groupvars=c('PMS'))
        ggplot(dataBig2, aes(x=1, y=Valence, fill=PMS))+
            geom_split_violin(adjust =1.5, alpha=.5)+
  ...
  output$int_means <- renderPlot({
    if ((grepl('Valence', input$formula_choice))){
      ...
      ggplot(dmean, aes(x=TestMoment, y=Valence, colour=PMS, group=PMS)) +
          geom_errorbar(aes(ymin=Valence-standard_error(Valence), ymax=Valence+standard_error(Valence), width=.1))+
        geom_line() +
        geom_point(size=3)
      } else {
      dmean<-summarySE(dataBig2, measurevar="Arousal", groupvars=c('TestMoment', 'PMS'))
    ggplot(dmean, aes(x=TestMoment, y=Arousal, colour=PMS, group=PMS)) +
     ...
     models <- reactive({
        m <- c()
        Formula <- paste0(input$formula_choice, '+ (1|Subject)')
        tryCatch({ d0.1 <- lmer(Formula,data=dataBig2); m <- c(m, d1=d0.1)}, error=function(e){})
        tryCatch({ d0.2 <- glmer(Formula,data=dataBig2, family = gaussian(link = "inverse"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d1=d0.2)}, error=function(e){})
        tryCatch({ d0.3 <- glmer(Formula,data=dataBig2, family = gaussian(link = "log"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d3=d0.3)}, error=function(e){})
        tryCatch({ d0.4 <- glmer(Formula,data=dataBig2, family = Gamma(link = "identity"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d4=d0.4)},error=function(e){})
        tryCatch({ d0.5 <- glmer(Formula,data=dataBig2, family = Gamma(link = "inverse"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d5=d0.5) }, error=function(e){})
        tryCatch({ d0.6 <- glmer(Formula,data=dataBig2, family = Gamma(link = "log"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d6=d0.6) }, error=function(e){})
        tryCatch({ d0.7 <- glmer(Formula,data=dataBig2, family = inverse.gaussian(link = "identity"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ);models <- c(models, d7=d0.7) }, error=function(e){})
        tryCatch({ d0.8 <- glmer(Formula,data=dataBig2, family = inverse.gaussian(link = "inverse"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d8=d0.8) }, 
          error=function(e){})
        tryCatch(+{ d0.9 <- glmer(Formula,data=dataBig2, family = inverse.gaussian(link = "log"),glmerfollicular(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ);  m <- c(m, d9=d0.9) }, 
          error=function(e){})
        m
     })

...
} else {"ERROR" #data from github dir
}
...
- correleer Valence met ValenceVideo. om dit te doen: zorg dat er geen lege plekken zijn in allPMSdata.csv ignore NA
...
![image link: https://imotions.com/blog/facial-action-coding-system/](https://media.imotions.com/images/20190411123347/AU12.gif)

**AU15** Lip Corner Depressor, Depressor anguli oris (Triangularis)

![](https://media.imotions.com/images/20190411123345/AU15.gif)
...
server <- function (input, output){ 
  #we make a density plot of valence/arousal per stimulus.
    ...
    output$correlations<- renderPlot({
      NAs<-sum(is.na(data$AU12))
        ggscatter(data, x='Valence', y='VideoValence', 
                        add='reg.line', fullrange=TRUE,
                    conf.int=TRUE,
                    cor.coef=TRUE, cor.method='pearson',
                  title=sprintf("\nNAs: %s\n", NAs),# NAs in AU12 & AU15

...
if (vpn == 1){
  dataDir = "Z:\\shares\\ghepmk_data\\2020_Kappen_PMS\\RMarkdown\\Data\\" #data from VPN folder
} else {
  dataDir = "Data\\" #data from github dir
}
data <- read.csv(paste0(dataDir,"allPMSdata.csv"), header=TRUE, sep=) #upload data
norms <-read.table(paste0(dataDir,"FemaleSubjects_1-20.txt"), header=TRUE, fill=TRUE) #upload norms IAPs images
...
ui<- fluidPage( # makes the User Interface
    selectInput("formula_choice", label="Choose a Formula:",
            choices= c('PSS ~ PMS', 'BSRI ~ PMS', 'PSS~ PMSS*TestMoment', 'BSRI ~PMSS*TestMoment'),
...
server <- function (input, output){ 
  ...
  output$raincloud <- renderPlot({
    if ((grepl("PSS", input$formula_choice))){
      p1 <- ggplot(data, aes(x = PMS, y = PSS, fill=PMS))+
      ...
      p2 <- ggplot(data, aes(x = PMS, y = BSRI, fill=PMS))+
        geom_flat_violin(position = position_nudge(x = .2, y = 0),alpha=.5,adjust =2)+
    ...
    output$splitviolin <-renderPlot({
      if ((grepl("PSS", input$formula_choice)==TRUE)&(grepl("PMS", input$formula_choice)==FALSE)){
              max_y<-max(as.integer(data$PSS))
        dmean<-summarySE(data, measurevar="PSS", groupvars=c('PMS'))
         p<- ggplot(data, aes(x=1, y=PSS, fill=PMS))+
           ...
           # annotate('text', x=1, y=max_y+(max_y/10)+(max_y/100), label='3', size=10)+
            ...
            ggtitle('PSS ~ PMS')
          p
      }else if (grepl('PSS', input$formula_choice)&(grepl("PMS", input$formula_choice)==TRUE)){
        max_y<- max(as.integer(data$PSS))
        dmean<-summarySE(data, measurevar="PSS", groupvars=c('PMS', 'Moment'))
          p<- ggplot(data, aes(x=Moment, y=PSS, fill=PMS))+
    ...
    } else if (grepl('BSRI', input$formula_choice)&(grepl("PMS", input$formula_choice)==TRUE)){
    max_y<-max(as.integer(data$BSRI))
  dmean<-summarySE(data, measurevar="BSRI", groupvars=c('PMS', 'Moment'))
    p<- ggplot(data, aes(x=Moment, y=BSRI, fill=PMS))+
      geom_split_violin(adjust =1.5, alpha=.5, colour= NA)+
    ...
    p
        ...
        dmean<-summarySE(data, measurevar="BSRI", groupvars=c('PMS'))
         p<- ggplot(data, aes(x=1, y=BSRI, fill=PMS))+
            geom_split_violin(adjust =1.5, alpha=.5)+
  ...
  #PMS vs noPMS over 2 fases Testmoment, https://github.com/jorvlan/raincloudplots
    output$raincloud <- renderPlot({
      if ((grepl("Stress", input$formula_choice))){
        p1 <- ggplot(data, aes(x = PMS, y = DASS_Stress, fill=PMS))+
        ...
        p2 <- ggplot(data, aes(x = PMS, y = DASS_Anxiety, fill=PMS))+
          geom_boxplot(width= .15, outlier.shape = NA)+
          geom_flat_violin(position = position_nudge(x = .2, y = 0),adjust =2)+
          geom_point(position=position_jitter(width=.15), size=.25)+
          ylab('Stress')+xlab('PMS')+coord_flip()+theme_cowplot()+
  ...
  ### Make split raincloud plot by just making two seperate rainclouds and plotting over each other! 
    ...
    output$splitviolin <- renderPlot({
      if ((grepl("Stress", input$formula_choice))){
        ...
        dmean<-summarySE(data, measurevar="DASS_Stress", groupvars=c('PMS', 'Moment'))
         p<- ggplot(data, aes(x=1, y=DASS_Stress, fill=PMS))+
            geom_split_violin(adjust =1.5, alpha=.5)+
        ...
        dmean<-summarySE(data, measurevar="DASS_Anxiety", groupvars=c('PMS', 'Moment'))
         p<- ggplot(data, aes(x=1, y=DASS_Anxiety, fill=PMS))+
            geom_split_violin(adjust =1.5, alpha=.5)+
        ...
        dmean<-summarySE(data, measurevar="DASS_Depression", groupvars=c('PMS', 'Moment'))
         p<- ggplot(data, aes(x=1, y=DASS_Depression, fill=PMS))+
            geom_split_violin(adjust =1.5, alpha=.5)+
  ...
  output$plotmeans <-renderPlot({
    if (grepl('Arousal', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
      ...
      dmean<-summarySE(dataBig2, measurevar="Valence", groupvars=c('PMS'))
        ...
        # p<- ggplot(dataBig2, aes(x=PMS, y=Valence, fill=PMS))+
        #   scale_x_discrete(breaks=c("0.5","1"))+
        #     geom_split_violin(aes(x=PMS, y=Valence),adjust=1.5,alpha=.5, colour=NA, width=1)+
        #   geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
        #   geom_boxplot(aes(x = PMS, y = Valence, fill = PMS),outlier.shape= NA, alpha = .5, width = .15, colour = "black")+
        #   geom_point(data = dmean, aes(x = as.numeric(PMS), y = Valence, group = PMS, fill = PMS), position=position_dodge(0.2), shape = 21, size=4)+
        #   ggtitle('Valence ~ PMS')+
        #   xlim(0,3)
        # p
...
-alle code opkuisen, plots toevoegen en nakijken, debuggen
-AIC table bij PSS!

  ...
  output$overlap <- renderPlot({
      if (grepl('PSS', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
        ...
        ggplot(data, aes(x=Moment, y=PSS, fill=PMS))+
          ...
          geom_point(data = dmean, aes(x = as.numeric(Moment)+0.1, y = PSS, fill= PMS), shape = 21, size=6)+
          geom_line(data = dmean, aes(x = as.numeric(Moment)+.1, y = PSS, group = PMS), linetype = 1)+

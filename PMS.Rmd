...
server <- function (input, output){ 
  p_pms <-reactive({
    p<-round(p()[[3]][1], digits=3) # this gets the p-value of the first row, for PMS
    star=c()
    if (p<0.05) {star='*'}
  ...
  p_moment <-reactive({
    p<-round(p()[[3]][2], digits=3) # this gets the p-value of the first row, for PMS
    star=c()
    if (p<0.05) {star='*'}
    else if (p<0.005){star='**'}
    else if (p<0.0005){star='***'}
  ...
  p_pms*mom <-reactive({
    p<-round(p()[[3]][3], digits=3) # this gets the p-value of the first row, for PMS
    star=c()
    if (p<0.05) {star='*'}
    else if (p<0.005){star='**'}
    else if (p<0.0005){star='***'}
...
-vul p-waarde in op de plotmeans voor enkelvoudige formule
-doe dit zo dat als p-w kleiner dan... zoveel sterretjes enz. 
-dan: doe hetzelfde mr voor de PMS*Moment. hacky manier: als.. bevat *, dan ...
-index juist en zet op figuur

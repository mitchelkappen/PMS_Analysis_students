...
rm(list = ls())
#install packages
list.of.packages <- c("lme4",'lmerTest','effects','dplyr', 'car', 'Rmisc', 'emmeans','fitdistrplus','ggplot2', 'ggstatsplot','ggsignif', 'remotes', 'ggformula','ggdist','ggpubr','ggeffects', 'gplots', 'gridExtra','tidyverse','pander','stringr','cowplot','lavaan','readr','jpeg','shiny','reshape2', 'yarrr','knitr', 'remotes', 'raincloudplots', 'corrplot')
...
data <- read.csv(paste0(Dir,"06102021\\cleanData.csv"), header=TRUE, sep=) #upload data
...
# data$PSS<-(data$PSS- min(data$PSS, na.rm=TRUE))/(max(data$PSS, na.rm=TRUE)-min(data$PSS, na.rm=TRUE))+0.000000001
# data$BSRI<-(data$BSRI- min(data$BSRI, na.rm=TRUE))/(max(data$BSRI, na.rm=TRUE)-min(data$BSRI, na.rm=TRUE))+0.000000001
# data$allRRS<-(data$allRRS- min(data$allRRS, na.rm=TRUE))/(max(data$allRRS, na.rm=TRUE)-min(data$allRRS, na.rm=TRUE))+0.000000001
# data$DASS_Depression<-(data$DASS_Depression- min(data$DASS_Depression, na.rm=TRUE))/(max(data$DASS_Depression, na.rm=TRUE)-min(data$DASS_Depression, na.rm=TRUE))+0.000000001
# data$DASS_Anxiety<-(data$DASS_Anxiety- min(data$DASS_Anxiety, na.rm=TRUE))/(max(data$DASS_Anxiety, na.rm=TRUE)-min(data$DASS_Anxiety, na.rm=TRUE))+0.000000001
# data$DASS_Stress<-(data$DASS_Stress- min(data$DASS_Stress, na.rm=TRUE))/(max(data$DASS_Stress, na.rm=TRUE)-min(data$DASS_Stress, na.rm=TRUE))+0.000000001


# 
# data$PSS<-data$PSS + 0.0000000000001
# data$BSRI <- data$BSRI  + 0.0000000000001
# data$allRRS<-data$allRRS  + 0.0000000000001
# data$DASS_Anxiety<-data$DASS_Anxiety + 0.0000000000001
# data$DASS_Stress<- data$DASS_Stress + 0.0000000000001
# data$DASS_Depression<- data$DASS_Depression + 0.0000000000001
    ...
    output$overlap <- renderPlot({
      if ((grepl("Depression", input$formula_choice))){
        ...
        ggplot(data, aes(x=PMS, y=DASS_Depression))+
              ...
              # geom_jitter(data=frame,aes(colour=PMS))+
              
              geom_point(data=data, aes(colour=PMS),position=position_jitter(width=.15), alpha=.5, size=1)+
              geom_boxplot(aes(x = PMS, y = DASS_Depression, fill = PMS), alpha= 0.2, outlier.shape= NA, width = .1, colour = "black")+
...
server <- function (input, output, session){ 
 ...
 output$table <-renderTable({
  ...
  noPMS1_PMDD1<- reactive ({p_function(2)})
  PMS1_PMDD1<- reactive ({p_function(6)})
  
  noPMS2_PMS2<- reactive ({p_function(13)})
  noPMS2_PMDD2<- reactive ({p_function(14)})
  PMS2_PMDD2<- reactive ({p_function(15)})

  
  noPMS1_noPMS2 <- reactive ({p_function(3)})
  PMS1_PMS2 <- reactive ({p_function(8)})
  PMDD1_PMDD2 <- reactive ({p_function(12)})
  ...
  output$overlap <- renderPlot({
      if (grepl('PSS', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
        ...
        ggplot(data, aes(x=Moment, y=PSS, fill=PMS))+
          ...
          {if(noPMS1_PMDD1()!='')geom_segment(aes(x =0.9, y = 2.9, xend = 1.1, yend = 2.9), size= 1)}+
            annotate('text', x=1, y=3, label=sprintf("\n%s\n", noPMS1_PMDD1()), size=10)+
  
            {if(noPMS2_PMS2()!='')geom_segment(aes(x =1.9, y = 2.5, xend = 2, yend = 2.5), size= 1)}+
            annotate('text', x=1.95, y=2.6, label=sprintf("\n%s\n", noPMS2_PMS2()), size=10)+
            {if(PMS2_PMDD2()!='')geom_segment(aes(x =2, y = 2.65, xend = 2.1, yend = 2.65), size= 1)}+
            annotate('text', x=2.05, y=2.7, label=sprintf("\n%s\n", PMS2_PMDD2()), size=10)+
          {if(noPMS2_PMDD2()!='')geom_segment(aes(x =1.9, y = 2.9, xend = 2.1, yend = 2.9), size= 1)}+
            annotate('text', x=2, y=3, label=sprintf("\n%s\n", noPMS2_PMDD2()), size=10)+
        ...
        ggplot(data, aes(x=Moment, y=BSRI, fill=PMS))+
          ...
          {if(noPMS2_PMDD2()!='')geom_segment(aes(x =1.9, y = 2.9, xend = 2.1, yend = 2.9), size= 1)}+
            ...
            {if(noPMS1_noPMS2()!='')geom_segment(aes(x =0.9, y = 3.3, xend = 1.9, yend = 3.3), size= 1, colour= "blue")}+
            annotate('text', x=1.4, y=3.4, label=sprintf("\n%s\n", noPMS1_noPMS2()), size=10)+
            {if(PMS1_PMS2()!='')geom_segment(aes(x =1, y = 3.6, xend = 2, yend = 3.6), size= 1, colour= "red")}+
            annotate('text', x=1.5, y=3.7, label=sprintf("\n%s\n", PMS1_PMS2()), size=10)+
            {if(PMDD1_PMDD2()!='')geom_segment(aes(x =1.1, y = 3.9, xend = 2.1, yend = 3.9), size= 1, colour= "purple")}+
            annotate('text', x=1.6, y=4, label=sprintf("\n%s\n", PMDD1_PMDD2()), size=10)+
...
server <- function (input, output){ 
  ...
  output$overlap <-renderPlot({
    if (grepl('Arousal', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
      ...
      #       geom_point(data = dmean, aes(x = as.numeric(PMS), y = Arousal, group=PMS, fill=PMS), shape = 21, size=6)+
      #       ggtitle('Arousal~PMS')+
      #       
      #       {if(noPMS_PMS()!='')geom_segment(aes(x = 1, y=2.5, xend= 2, yend=2.5), size= 1)}+
      #         annotate('text', x=1.5, y=2.6, label=sprintf("\n%s\n", noPMS_PMS()), size=10)+
      ...
      max_y<-max(as.integer(data$Arousal))
            dmean<-summarySE(data, measurevar="Arousal", groupvars=c('PMS'), na.rm=TRUE)
          ggplot(data, aes(x = PMS, y = Arousal)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = 0), adjust = 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
            geom_boxplot(aes(x = PMS, y = Arousal, fill = PMS),outlier.shape= NA, width = .1, colour = "black")+
          ...
          # annotate('text', x=1.5, y=max_y+(max_y/10)+(max_y/50), label=sprintf("\n%s\n", p_pms()), size=10)+
            ...
            ggtitle('Arousal~PMS')+
        scale_fill_manual(values = c("red1", "#49A4B9", "#2C5985"))+
            scale_colour_manual(values = c("red1", "#49A4B9", "#2C5985"))+
          theme_bw()
    }else {
      dmean<-summarySE(data, measurevar="Valence", groupvars=c('PMS'), na.rm=TRUE)
          ggplot(data, aes(x = PMS, y = Valence)) +
            geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = 0), adjust = 1.5, alpha = .5, colour = NA)+
            geom_point(aes(colour=PMS),position=position_jitter(width=.15), size=.25)+
            geom_boxplot(aes(x = PMS, y = Valence, fill = PMS),outlier.shape= NA, width = .1, colour = "black")+
            geom_point(data = dmean, aes(x = as.numeric(PMS), y = Valence, group=PMS, fill=PMS), shape = 21, size=6)+
            # geom_segment(aes(x = 1, y=max_y+(max_y/10)+(max_y/150), xend = 2, yend=max_y+(max_y/10)+(max_y/150)), size= 1.7)+
          # annotate('text', x=1.5, y=max_y+(max_y/10)+(max_y/50), label=sprintf("\n%s\n", p_pms()), size=10)+
            scale_x_discrete(labels=c('1'='follicular', '2'='premenstrual'))+
            ggtitle('Valence~PMS')+
  ...
  output$overlap <-renderPlot({
    if (grepl('arousal', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
     ...
     models <- reactive({
        ...
        tryCatch({ d0.2 <- glmer(Formula,data=datafaces, family = gaussian(link = "inverse"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d2=d0.2)}, error=function(e){})
        tryCatch({ d0.3 <- glmer(Formula,data=datafaces, family = gaussian(link = "log"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d3=d0.3)}, error=function(e){})
        tryCatch({ d0.4 <- glmer(Formula,data=datafaces, family = Gamma(link = "identity"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d4=d0.4)},error=function(e){})
        tryCatch({ d0.5 <- glmer(Formula,data=datafaces, family = Gamma(link = "inverse"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d5=d0.5) }, error=function(e){})
        tryCatch({ d0.6 <- glmer(Formula,data=datafaces, family = Gamma(link = "log"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d6=d0.6) }, error=function(e){})
        tryCatch({ d0.7 <- glmer(Formula,data=datafaces, family = inverse.gaussian(link = "identity"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ);models <- c(models, d7=d0.7) }, error=function(e){})
        tryCatch({ d0.8 <- glmer(Formula,data=datafaces, family = inverse.gaussian(link = "inverse"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ); m <- c(m, d8=d0.8) }, 
          error=function(e){})
        tryCatch(+{ d0.9 <- glmer(Formula,data=datafaces, family = inverse.gaussian(link = "log"),glmerControl(optimizer= "bobyqa", optCtrl = list(maxfun = 100000)),nAGQ=nAGQ);  m <- c(m, d9=d0.9) }, 
          error=function(e){})
        m
     })
...
exploratory: contraception ²


R MARKDOWN

question: is the translation to dutch valid? this study could attest to that!

=> heropen python bestand! (github?)
rauwe data bekijken

herbekijk PSST paper: hoe onderverdeeld obv vragen? (kijk tilia en liese)
verdeel in noPMS, PMS en PMDD, en geef descriptives voor elke groep 
plot deze drie over tijd (voeg de formules toe)

...
server <- function (input, output){ 
 ...
 output$table <-renderTable({
  ...
  noPMS1_noPMS2 <- reactive ({emtable()[3,6]})
  PMS1_PMS2 <- reactive ({emtable()[7,6]})
  PMDD1_PMDD2 <- reactive ({emtable()[12,6]})
  ...
  output$overlap <- renderPlot({
      if (grepl('PSS', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
        ...
        ggplot(data, aes(x=Moment, y=PSS, fill=PMS))+
          ...
          geom_segment(aes(x =0.9, y = 2.5, xend = 1, yend = 2.5), size= 2)+
          geom_segment(aes(x =1, y = 2.7, xend = 1.1, yend = 2.7), size= 2)+

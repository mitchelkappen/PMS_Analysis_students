...
server <- function (input, output){ #we have to add this to the server
  ...
  output$table <- renderTable ({
    Formula <- paste0('DASS_Stress ~ PMSScoreNew', ' + (1|Subject)')
    ...
    ##kick out automatically
    #try functions gebruiken: probeert iets, als zonders errors lukt doet die het, ander sniet
    #dingen leeg maken van tevoren. als d0.1 niet werkt, clearen. 
    #tablebind 
    #try functie: try and catch. probeer als werkt catch en dan volgde: soort if else met foutmelding. dan toevoegen aan list modelnames, dan uitgangspositie rest

...
#install packages
list.of.packages <- c("lme4",'lmerTest','effects','dplyr', 'car', 'emmeans','fitdistrplus','ggplot2', 'ggstatsplot','ggsignif', 'ggformula','ggdist','ggpubr','ggeffects', 'gplots', 'gridExtra','tidyverse','pander','stringr','cowplot','lavaan','readr','jpeg','shiny','reshape2', 'yarrr','knitr' )
...
```{r echo=FALSE}
d<-melt(data.frame(
  valence=c(dataBig2$Valence[unique(dataBig2$ID)]),
  arousal=c(dataBig2$Arousal[unique(dataBig2$ID)])
  ))
...
server <- function (input, output){ 
  ...
  output$plot<- renderPlot({
    ...
    output$plot2 <- renderPlot({
      plotmeans(input$formula_choice, data=dataBig2)
    })
  })
...
-devide stimuli groups on valence (read about IAP images)

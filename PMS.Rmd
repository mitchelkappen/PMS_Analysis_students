  ...
  output$overlap <- renderPlot({
      if (grepl('PSS', input$formula_choice) &grepl('TestMoment', input$formula_choice)){
        ...
        ggplot(data, aes(x=Moment, y=PSS, fill=PMS))+
          ...
          # geom_line(data = dmean, aes(x = as.numeric(Moment)+.3, y = PSS, group = PMS), linetype = 1)+
          ...
          annotate('text', x=1.4, y=2.8, label=sprintf("\n%s\n", noPMS1_noPMS2()), size=10)+
          geom_segment(aes(x =1, y = 3.5, xend = 2, yend = 3.5), size= 1, colour= "#49A4B9")+
          annotate('text', x=1.5, y=2.8, label=sprintf("\n%s\n", PMS1_PMS2()), size=10)+
          geom_segment(aes(x =1.1, y = 4, xend = 2.1, yend = 4), size= 1, colour= "#92C5DE")+
          annotate('text', x=1.6, y=2.8, label=sprintf("\n%s\n", PMDD1_PMDD2()), size=10)+
          
          geom_point(data = dmean, aes(x = as.numeric(Moment), y = PSS, group = PMS, fill = PMS), position= position_dodge(0.3), shape = 21, size=5)+

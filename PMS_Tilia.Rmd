...
norms <- read.table(paste0(Dir,"\\RMarkdown\\Data\\FemaleSubjects_1-20.txt"), header=TRUE, fill=TRUE) # upload norms
...
# I don't quite understand the FemaleSubjects_1-20.txt" used in Sofies markdown ?????????????????? ask

IAPS <- data_cleanedrt %>%
  group_by(ID) %>%D
  group_by(PMSbinary, TestMoment) %>%

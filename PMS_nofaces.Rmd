...
d <- read.csv(paste0(Dir,"06102021\\allPMSData.csv"), header=TRUE, sep=) #upload data 
...
data$allRRS <- as.integer((data$allRRS - mean(data$allRRS, na.rm=TRUE))/sd(data$allRRS, na.rm=TRUE))
data$DASS_Depression <- as.integer((data$DASS_Depression - mean(data$DASS_Depression, na.rm=TRUE))/sd(data$DASS_Depression, na.rm=TRUE))
data$DASS_Anxiety <- as.integer((data$DASS_Anxiety - mean(data$DASS_Anxiety, na.rm=TRUE))/sd(data$DASS_Anxiety, na.rm=TRUE))
data$DASS_Stress <- as.integer((data$DASS_Stress - mean(data$DASS_Stress, na.rm=TRUE))/sd(data$DASS_Stress, na.rm=TRUE))
...
server <- function (input, output, session){ 
   ...
   models <- reactive({ # we need to create this within a reactive context to be able to access the formula_choice
        ...
        # Formula <- DASS_Depression ~ PMS + (1|Subject)
...
#input with 
# noPMS, PMS of PMDD group
#var one is PSS, BSRI, 1_PSS, 1_BSRI, 2_PSS, 2_BSRI, delta_PSS, delta_BSRI, var 2 is ..
ui<- fluidPage( # makes the User Interface
    selectInput("group", label="group",
            choices= c('noPMS', 'PMS', 'PMDD'),
            selected='noPMS'),
    selectInput("var1", label = 'var1',
                choices = c('PSS', 'BSRI', '1_PSS', '1_BSRI', '2_PSS', '2_BSRI', 'delta_PSS', 'delta_BSRI'), selected = 'PSS'),
    selectInput("var2", label = 'var2',
                choices = c('PSS', 'BSRI', '1_PSS', '1_BSRI', '2_PSS', '2_BSRI', 'delta_PSS', 'delta_BSRI'), selected = 'PSS'),
     tabsetPanel(type = "tab",
                 tabPanel("corrplot",plotOutput("corrplot")),
...
output$corrplot <- renderPlot({
  if ((grepl("noPMS", input$group)==TRUE)){
    noPMScor<- cor(noPMSframe)
  corrplot(noPMScor, method = 'color')
  }
  else if ((grepl("PMS", input$group)==TRUE)){
    PMScor<- cor(PMSframe)
  corrplot(PMScor, method = 'color')
  }
  else if ((grepl("PMDD", input$group)==TRUE)){
    PMDDcor<- cor(PMDDframe)
  corrplot(PMDDcor, method = 'color')
  }
}, height=700)

# output$var1 <- renderText({as.character(input$var1)})
group <- reactive ({as.character(input$group) })
var1 <- reactive ({ paste(group(), as.character(input$var1), sep="_") })
var2 <- reactive ({paste(group(), as.character(input$var2), sep="_") })
# output$var2 <- renderText({as.character(input$var2)})
  
output$scatterplot <- renderPlot({
  library(ggpubr)
    if ((grepl("noPMS", input$group)==TRUE)){
        ggscatter(noPMSframe, x = var1(), y = var2(),
        ...
        ggscatter(PMSframe, x = var1(), y = var2(),
                        add='reg.line', fullrange=TRUE,
                    conf.int=TRUE,
                    cor.coef=TRUE, cor.method='pearson',
                    xlab=var1(), ylab=var2())+
        ...
        ggscatter(PMDDframe, x = var1(), y = var2(),
                        add='reg.line', fullrange=TRUE,
                    conf.int=TRUE,
                    cor.coef=TRUE, cor.method='pearson',
                    xlab=var1(), ylab=var2())+
  ...
  cor.test(PMS_delta_PSS, PMS_1_BSRI, method="pearson", use="complete.obs")

...
traitplot <-function(data, emmean_dataframe, var, title){
  ggplot(data, aes(x = PMS, y = .data[[var]])) +
    geom_flat_violin(aes(fill=PMS),position = position_nudge(x =.2, y = 0), adjust = 1.5)+
    geom_boxplot(aes(x = PMS, y = .data[[var]], fill = PMS), outlier.shape=NA, alpha= .45, width = .15, colour = "black") +
    geom_point(data= emmean_dataframe, aes(x = PMS, y = emmean, fill=PMS), size=4)+
    scale_colour_manual(values = c("salmon", "red", "darkred"))+
...
stateplot <-function(data, emmean_dataframe, var, title){
  ggplot()+ 
    geom_flat_violin(data= data, aes(x= Moment, y= .data[[var]], fill=PMS),position = position_nudge(x =.3, y = 0), adjust = 1.5, alpha = .5)+ # flat violin distribution, .3 points to the right. alpha=.5 so see-through
    geom_boxplot(data= data, aes(x=Moment, y=.data[[var]], fill=PMS), outlier.shape=NA, alpha=.7, width=.3, colour='black')+ #boxplot, see through, no outline, 
    ...
    scale_fill_manual(values = c("salmon", 'red', 'darkred'), #colours used in plot, repressent PMDD, PMS and noPMS
                      name='', #legend gets no name
                      labels=c('noPMS','PMS','PMDD'))+ #labels names
    guides(fill = guide_legend(reverse=TRUE))+ # show labels in different order 
    labs(y=title)+
    scale_x_discrete(labels=c("Follicular", "Luteal"))+
    theme(
      legend.key.size=unit(1.5, 'cm'), # make keys of legend bigger

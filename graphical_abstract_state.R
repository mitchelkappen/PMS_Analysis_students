...
source("graphical_abstract_functions.R") # This is a file in the same directory where you can stash your functions so you can save them there and have them together
...
ggsave(plot, file=paste0(plotPrefix, "PSS_Plot.jpeg"), width = 2500, height = 1100, dpi = 300, units = "px") # save plot
plot


ggplot(data, aes(x=Moment, y=PSS, fill=PMS), dpi=1500)+
  geom_split_violin(adjust =1.5, alpha=.7, colour= NA)+
  geom_boxplot(aes(x = Moment, y = PSS, fill = PMS), position= position_dodge(0.2),outlier.shape= NA, alpha = .5, width = .1, colour = "black")+
  geom_line(data = emm0.2, aes(x = Moment, y = emmean, fill=PMS), position=position_dodge(0.2), linetype = 1)+
  geom_point(data = emm0.1, aes(x = Moment, y = emmean), position= position_dodge(0.2), shape = 21, size=4)+
...
## Visualisation
max_y<-max(data$PTQ)
plot <- stateplot(data, emm0.2, "PTQ", 'Rumination') +
  # Follicular
